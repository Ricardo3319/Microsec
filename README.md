...
基于实际运行结果，Malcolm-Strict 在 8 节点异构集群上的表现：

| 指标 | Exp A (Po2) | Exp B (理想) | Exp C (Malcolm-Strict) | 相对改进 |
|------|-----------|-------------|---------------------|---------|
| **吞吐量 (RPS)** | 6,966 | 42,865 | **32,325** | **364%** ↑ |
| **P99 延迟 (ms)** | 24.08 | 2.29 | **3.70** | **84.6%** ↓ |
| **违约率** | 60.13% | 0.01% | **0.02%** | **3000x** ↓ |
| **相对性能** | - | 100% | **75.4%** | - |

### 关键发现

**1. 方差陷阱实证**：
- 仅加入 3 个慢节点，Po2 性能暴跌 83.7%
- 延迟恶化 10.5 倍，完全失效

**2. Malcolm-Strict 有效性**：
- 364% 吞吐量提升，84.6% 延迟降低
- 违约率从 60.13% 清零到 0.02%
- 机制：Slack Histogram 感知异构性，IQN/CVaR 风险决策，EDF 有序执行

**3. 物理极限逼近**：
- 利用仅 40% 快速资源，达到 75% 理想吞吐量
- P99 延迟仅为理想的 1.61 倍，接近调度物理极限

### 实验时间线

- **2026.01.05**：首次运行，eRPC TLS 崩溃
- **2026.01.06**：完成工程创新 1-3，系统稳定
